...
var passwordFunc = require('./cs142password.js');

var versionString = '1.0';

// We start by removing anything that existing in the collections.
var removePromises = [User.deleteMany({}), Photo.deleteMany({}), SchemaInfo.deleteMany({}), Feed.deleteMany({})];
...
Promise.all(removePromises).then(function () {
    ...
    var userPromises = userModels.map(function (user) {
        let passObj = passwordFunc.makePasswordEntry('weak');
        console.log();
        console.log();
        console.log(passObj);
        return User.create({
